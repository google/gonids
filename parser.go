...
//validate every item is bettwen 1 and 65535
...
func iPAddressesValid(ip ...string) bool {
	for _, _ip := range ip {
		if len(_ip) > 0 && string(_ip[0:1]) == "!" {
			_ip = _ip[1:]
		}
		if _ip == "any" || strings.HasPrefix(_ip, "$") {
			continue
		}
